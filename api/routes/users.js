...
router.delete("/:id", async(req, res)=>{
    ...
    if(req.body.userId === req.params.id || req.body.isAdmin){
        ...
        return res.status(403).send("You can delete update your account")
    }
});
//get a user
router.get("/", async(req, res)=>{
    ...
    try{
        ...
        //filtering the response object by removing the password and updatedAt properties
    ...
    if(req.body.userId !== req.params.id){
        try{
            ...
            if(user.followers.includes(req.body.userId)){
                ...
                return res.status(403).json("not following user")
            }
        }catch(err){
            return res.status(500).json(err)
        }
